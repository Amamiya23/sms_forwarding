...
{
    // 手填优先：CNUM 在部分 eSIM/漫游卡上会报错号（如 TOA 0x91 被解成 +19…）
...
static bool process_smtp_test_one()
{
    ...
    data.title = "SMTP 配置测试";
    data.message = "设备已用当前 SMTP 参数发送测试邮件。若收到此信，说明配置可用。";
    data.timestamp = format_local_time(idf_config_get_tz_offset());
    if (data.timestamp.empty()) data.timestamp = "时间未同步";

    s_busy.store(true, std::memory_order_relaxed);
    bool ok = send_smtp_email(cfg, data);
    s_busy.store(false, std::memory_order_relaxed);
