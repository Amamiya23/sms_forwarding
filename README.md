...
- ML307 系列 4G/LTE 模组。
- USB 数据线和对应串口驱动。
- ESP-IDF 6.0（当前验证版本为 6.0.2）。仓库脚本会优先读取 Espressif Installation Manager 的 `C:\Espressif\tools\eim_idf.json`，也可通过 `IDF_PATH`、`IDF_TOOLS_PATH` 和 `IDF_PYTHON_ENV_PATH` 环境变量指定自己的安装位置。
